...
class HooksController {
    _getInvoice(login) {
        return new Promise((resolve) => {
            axios_1.default
                .get(`${Env_1.default.get('AMEMBER_URL')}/api/check-access/by-login`, {
                params: {
        ...
        const results = await Database_1.default.transaction(async (t) => {
            ...
            for (let item of items) {
                ...
                if (item.isSub && categories['1']) {
                    ...
                    user.subscriptionEnd = luxon_1.DateTime.fromObject({
                        year: splitDate[0],
                        month: splitDate[1],
                        day: splitDate[2],
                    });
    ...
    async updateUser({ request, response }) {
        ...
        let user = await User_1.default.findBy('amember_id', parseInt(amemberId));
        if (!user) {
        ...
        let axiosResponse;
        try {
            axiosResponse = await axios_1.default.get(`${Env_1.default.get('AMEMBER_URL')}/api/check-access/by-login`, {
                params: {
        ...
        if (parseInt(amemberId) !== parseInt(axiosResponse.data.user_id))
            return response.badRequest();
    ...
    async productHook({ request, response }) {
        ...
        let responseAxios;
        try {
            const getData = (await axios_1.default.get(`${Env_1.default.get('AMEMBER_URL')}/api/products/${amemberId}?_key=${Env_1.default.get('AMEMBER_KEY')}&_nested[]=billing-plans&_nested[]=product-product-category`)).data;
        ...
        if (category === '2') {
            let course = await Course_1.default.findBy('amember_id', amemberId);
            if (!course) {
            ...
            let product = await Product_1.default.findBy('amember_id', amemberId);
            if (!product) {
